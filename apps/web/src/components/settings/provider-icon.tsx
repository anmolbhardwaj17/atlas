/** Brand marks for the BYO-LLM providers (not in the cloud-icon set). Each uses its brand color
 *  so the provider selector reads at a glance. OpenAI's mark is monochrome (inherits foreground). */
export function ProviderIcon({ id, className }: { id: string; className?: string }) {
  ...
  return (
    ...
    >
      ...
      <circle cx="19.5" cy="17.5" r="1.6" fill="currentColor" stroke="none" />
      <path d="M6 12h4l4-5.5h3.5M10 12l4 5.5h3.5" />
    </svg>
  );
}
