...
// Core platform tables (docs/04 §5.1). One statement per execute() — Kysely uses
// the extended protocol, which forbids multi-statement strings.
...
export async function up(db: Kysely<unknown>): Promise<void> {
  ...
  // ── users (global identity; mirrors Supabase auth.users — docs/12) ──────────
  await sql`
    CREATE TABLE users (
      id          uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  ...
  // ── memberships (org-scoped; unit of authorization) ─────────────────────────
  await sql`
  ...
  await sql`
    CREATE TRIGGER trg_mem_updated BEFORE UPDATE ON memberships
      FOR EACH ROW EXECUTE FUNCTION set_updated_at()
  `.execute(db);
  await sql`CREATE INDEX ix_memberships_user ON memberships(user_id)`.execute(db);
...
export async function down(db: Kysely<unknown>): Promise<void> {
  await sql`DROP TABLE IF EXISTS invitations, memberships, auth_identities, users, organizations CASCADE`.execute(
    db,
  );
