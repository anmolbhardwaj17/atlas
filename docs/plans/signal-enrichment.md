...
> **Status:** 🔵 In progress — S1 tags/R11 ✅ · S2 image→SHA/R12 ✅ · S4 service-name env/R13 ✅ shipped; S3 (IaC), S5 (log-content), S6 (semantic+confirm) queued.
...
| **S4** | Observability service names (R13) | `OTEL_SERVICE_NAME` / `DD_SERVICE` / `SERVICE_NAME` / OTEL resource attrs env values | `DEPLOYS_TO` repo→runtime | exact normalized equality to a repo slug; unique→high, ambiguous→low, generic skipped | none new (reuses `aws.lambda.env`/`aws.ecs.env` signals) | ✅ **built** |
