...
- 🏗️ Build: **F1 ✅ + F2 ✅ COMPLETE** (Phase F done). **Now in Phase I (Ingest):** I1 (AWS crawler) ∥ I2 (GitHub crawler) implement the frozen SDK (`docs/06`/`07`). **I1 (AWS crawler) CODE-COMPLETE** (I1.1–I1.4; live-account verify pending creds). **Phase I CODE-COMPLETE** (I1 AWS + I2 GitHub, live verify pending creds). **Now on Phase G: G1.1 ✅** engine + signals + R1 · **G1.2 ✅** R4 (atlas.service + IMPLEMENTS/RUNS) + R5-prop (service OWNED_BY) + R6 (CHANGED_BY) — the full worked-example chain reproduces on live DB. Next: G1.3 (R2 SG-correlation + R3 env-refs + R8 IAM → CONNECTS_TO/STORES_IN).
...
### Sprint G1 — Inference engine + edges *(`docs/05` §6)*
...
| G1.3 | R2 (SG correlation) + R3 (env refs) + R8 (IAM access) → CONNECTS_TO/STORES_IN | 📋 | 05 §6.4 | |
| G1.4 | Reconciliation/determinism golden files + precision harness ≥95% + wire infer stage | 📋 | 05 §6.5, 14 §10 | reproduce §9 worked example |
