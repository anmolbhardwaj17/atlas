...
- 🏗️ Build: **F1 ✅ + F2 ✅ COMPLETE** (Phase F done). **Now in Phase I (Ingest):** I1 (AWS crawler) ∥ I2 (GitHub crawler) implement the frozen SDK (`docs/06`/`07`). **I1 (AWS crawler) CODE-COMPLETE** (I1.1–I1.4; live-account verify pending creds). **Phase I CODE-COMPLETE** (I1 AWS + I2 GitHub, live verify pending creds). **Now on Phase G (Graph & Intelligence): G1.1 ✅** — `@atlas/inference` engine (pure versioned rules → inferred edges with provenance/evidence; convergent retire) + `signals` table persisted during sync + **R1 DEPLOYS_TO** (tiered by match strength). Next: G1.2 (R4 atlas.service + R6 CHANGED_BY + R5-propagation).
...
| G1.2 | R4 (atlas.service + IMPLEMENTS/RUNS) + R6 (CHANGED_BY) + R5-propagation | 📋 | 05 §6.4 | next |
