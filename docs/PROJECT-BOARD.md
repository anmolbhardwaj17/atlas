...
- 🏗️ Build: **F1 ✅ + F2 ✅ COMPLETE** (Phase F done). **Now in Phase I (Ingest):** I1 (AWS crawler) ∥ I2 (GitHub crawler) implement the frozen SDK (`docs/06`/`07`). **I1.1–I1.4 ✅ — I1 (AWS crawler) CODE-COMPLETE** (live-account verify pending creds): foundation + URN/vocab · AssumeRole + verify/degraded · 14 service modules (every observed-edge type, golden fixtures) · 14 live discoverers + retry/rate-limit + registry wiring + enqueue-on-verify + incremental hash-diff. Next: **I2 (GitHub crawler)** (`docs/07`).
