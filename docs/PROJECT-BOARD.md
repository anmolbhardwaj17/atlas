...
- 🏗️ Build: **F1 ✅ + F2 ✅ COMPLETE** (Phase F done). **Now in Phase I (Ingest):** I1 (AWS crawler) ∥ I2 (GitHub crawler) implement the frozen SDK (`docs/06`/`07`). **I1 (AWS crawler) CODE-COMPLETE** (I1.1–I1.4; live-account verify pending creds). **Now on I2 (GitHub crawler):** **I2.1 ✅** foundation · **I2.2 ✅** GitHub App installation-token auth + verify/health + permission-detection→degraded. Next: I2.3 (pure parsers: workflows→deploy signals, CODEOWNERS→OWNED_BY, manifests→DEPENDS_ON_PKG, golden fixtures).
...
| I2.3 | Pure parsers + normalize/extractSignals/observedEdges (golden fixtures) | 📋 | 07 §6/§7, 05, 14 | workflows→deploy signals, CODEOWNERS→OWNED_BY, manifests→DEPENDS_ON_PKG |
