...
- 🏗️ Build: **F1 ✅ + F2 ✅ COMPLETE** (Phase F done). **Now in Phase I (Ingest):** I1 (AWS crawler) ∥ I2 (GitHub crawler) implement the frozen SDK (`docs/06`/`07`). **I1 (AWS crawler) CODE-COMPLETE** (I1.1–I1.4; live-account verify pending creds). **Phase I CODE-COMPLETE — both crawlers built (CI-green):** I1 (AWS, `@atlas/connector-aws`) + I2 (GitHub, `@atlas/connector-github`). I2.1 foundation · I2.2 App auth + verify/degraded · I2.3 pure parsers + 6 entity modules (OWNED_BY/DEPENDS_ON_PKG/CONTAINS + signals) · I2.4 live client (pagination/rate-limit) + crawl + webhook HMAC + registry wiring. **Open: live verification (needs customer creds).** Next: **G1 (inference engine + edges)**.
