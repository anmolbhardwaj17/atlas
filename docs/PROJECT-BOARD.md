...
- 🏗️ Build: **F1 ✅ + F2 ✅ COMPLETE** (Phase F done). **Now in Phase I (Ingest):** I1 (AWS crawler) ∥ I2 (GitHub crawler) implement the frozen SDK (`docs/06`/`07`). **I1 (AWS crawler) CODE-COMPLETE** (I1.1–I1.4; live-account verify pending creds). **Now on I2 (GitHub crawler):** **I2.1 ✅** foundation (`@atlas/connector-github`: URN grammar + node-kind catalog; migration 0009 seeds `external.package`). Next: I2.2 (GitHub App installation-token auth + verify + degraded).
...
| I2.2 | GitHub App auth (installation token) + verify/health + permission detection | 📋 | 07 §2, 12, 13 §5 | next; App JWT→installation token |
