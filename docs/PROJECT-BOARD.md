...
- 🏗️ Build: **F1 ✅ + F2 ✅ COMPLETE** (Phase F done). **Now in Phase I (Ingest):** I1 (AWS crawler) ∥ I2 (GitHub crawler) implement the frozen SDK (`docs/06`/`07`). **I1 (AWS crawler) CODE-COMPLETE** (I1.1–I1.4; live-account verify pending creds). **Now on I2 (GitHub crawler):** **I2.1 ✅** foundation · **I2.2 ✅** App auth + verify/degraded · **I2.3 ✅** pure parsers (CODEOWNERS/manifests/workflows) + 6 entity modules (OWNED_BY, DEPENDS_ON_PKG, CONTAINS + deploy/PR signals), golden fixtures. Next: I2.4 (live Octokit discoverers + rate-limit/retry + webhook HMAC ingress + wire into runner/registry/API).
...
> **I1 (AWS crawler) is CODE-COMPLETE** (I1.1–I1.4) — the only open item is **live-account verification**, which needs a customer read-only IAM role (role ARN + Atlas-generated External ID, docs/13 §4). When creds exist: create the connection, verify (→ connected/degraded + missing-perms), confirm populated nodes/edges/provenance + raw snapshots. **Additive service follow-ups** (no architecture change, docs/06 §9): DynamoDB, ElastiCache, API Gateway, IAM-policy. **Next: I2 (GitHub crawler).**
...
| I2.4 | Live discoverers (Octokit) + rate-limit/retry + webhook HMAC ingress + wiring | 📋 | 07 §5/§10, 13 §5 | replaces GitHub placeholder; enqueue-on-verify |
